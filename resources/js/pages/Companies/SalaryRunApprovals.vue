...
const breadcrumbs = computed((): BreadcrumbItem[] => [
    ...
    },
    {
        ...
        href: route('companies.salary-run-approvals.index', props.company.id),
    },
]);
...
const submitCreate = () => {
    createForm.post(route('companies.salary-run-approvals.store', props.company.id), {
        preserveScroll: true,
        onSuccess: () => {
            createForm.reset();
        },
...
const submitEdit = (stepId: number) => {
    editForm.put(route('companies.salary-run-approvals.update', [props.company.id, stepId]), {
        preserveScroll: true,
        onSuccess: () => {
            editingStepId.value = null;
            editForm.reset();
...
const deleteStep = (step: StepItem) => {
    ...
    router.delete(route('companies.salary-run-approvals.destroy', [props.company.id, step.id]), {
        preserveScroll: true,
    });
};
...
const moveStep = (index: number, direction: -1 | 1) => {
    ...
    router.post(route('companies.salary-run-approvals.reorder', props.company.id), { ordered_ids: orderedIds }, {
        preserveScroll: true,
    });
};
</script>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="max-w-4xl mx-auto px-4 py-8 space-y-6">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <HeadingSmall
                    :title="t('settings.salary_run_approvals_for_company', { company: companyName })"
