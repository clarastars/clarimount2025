...
const breadcrumbs = computed((): BreadcrumbItem[] => [
    ...
    },
    {
        ...
        href: route('companies.salary-certificate-approvals.index', props.company.id),
    },
]);
...
const submitCreate = () => {
    createForm.post(route('companies.salary-certificate-approvals.store', props.company.id), {
        preserveScroll: true,
        onSuccess: () => {
            createForm.reset();
        },
...
const submitEdit = (stepId: number) => {
    editForm.put(route('companies.salary-certificate-approvals.update', [props.company.id, stepId]), {
        preserveScroll: true,
        onSuccess: () => {
            editingStepId.value = null;
            editForm.reset();
...
const deleteStep = (step: StepItem) => {
    if (!window.confirm(t('settings.salary_certificate_approvals_delete_confirm'))) {
        return;
    }

    router.delete(route('companies.salary-certificate-approvals.destroy', [props.company.id, step.id]), {
        preserveScroll: true,
    });
};
...
const moveStep = (index: number, direction: -1 | 1) => {
    ...
    router.post(route('companies.salary-certificate-approvals.reorder', props.company.id), { ordered_ids: orderedIds }, {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <AppLayout :breadcrumbs="breadcrumbs">
        <Head :title="t('settings.salary_certificate_approvals')" />

        <div class="max-w-4xl mx-auto px-4 py-8 space-y-6">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <HeadingSmall
                    :title="t('settings.salary_certificate_approvals_for_company', { company: companyName })"
                    :description="t('settings.salary_certificate_approvals_description')"
                />
            ...
            <Card>
                <CardHeader>
                    <CardTitle>{{ t('settings.salary_certificate_approvals_add_step') }}</CardTitle>
                </CardHeader>
                <CardContent>
                    <form class="space-y-4" @submit.prevent="submitCreate">
                        <div class="space-y-2">
                            <Label for="new-title">{{ t('settings.salary_certificate_approval_step_title') }}</Label>
                            <Input id="new-title" v-model="createForm.title" required />
                            <InputError :message="createForm.errors.title" />
                        </div>
                        <div class="space-y-2">
                            <Label for="new-team">{{ t('settings.salary_certificate_approval_team') }}</Label>
                            <select
                                id="new-team"
                                ...
                                required
                            >
                                <option value="" disabled>{{ t('settings.salary_certificate_approval_team_placeholder') }}</option>
            ...
            <Card>
                <CardHeader>
                    <CardTitle>{{ t('settings.salary_certificate_approvals_steps_list') }}</CardTitle>
                </CardHeader>
                <CardContent class="space-y-4">
                    <p v-if="steps.length === 0" class="text-sm text-muted-foreground">
                        {{ t('settings.salary_certificate_approvals_empty') }}
                    </p>
                    ...
                    >
                        <div v-if="editingStepId === step.id" class="space-y-3">
                            <div class="space-y-2">
                                <Label>{{ t('settings.salary_certificate_approval_step_title') }}</Label>
                                <Input v-model="editForm.title" required />
                                <InputError :message="editForm.errors.title" />
                            </div>
                            <div class="space-y-2">
                                <Label>{{ t('settings.salary_certificate_approval_team') }}</Label>
                                <select
