...
const form = useForm({
    ...
    workdays: weekdays.map((w) => {
        ...
        return {
            weekday: w.weekday,
            is_workday: existing ? existing.is_workday : true,
            start_time: existing?.start_time ? toTimeStr(String(existing.start_time)) : '',
            end_time: existing?.end_time ? toTimeStr(String(existing.end_time)) : '',
        }
    }),
})
