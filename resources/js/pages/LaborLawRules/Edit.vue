...
interface LaborLawRule {
    ...
    action_value_gross_days: number | null
    action_value_basic_days: number | null
    reason_text: string
}
...
const breadcrumbs = computed((): BreadcrumbItem[] => [
    ...
    {
        title: t('labor_law_rules.edit_rule'),
        href: `/labor-law-rules/${props.rule.id}/edit`,
    },
])

const form = useForm({
    violation_type: props.rule.violation_type,
    min_minutes: props.rule.min_minutes,
    max_minutes: props.rule.max_minutes,
    repeat_number: props.rule.repeat_number,
    action_type: props.rule.action_type,
    action_value: props.rule.action_value,
    action_value_gross_days: props.rule.action_value_gross_days,
    action_value_basic_days: props.rule.action_value_basic_days,
    reason_text: props.rule.reason_text,
})
...
const submit = () => {
    form.put(route('labor-law-rules.update', props.rule.id))
}
</script>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="py-12">
            <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
                <Card>
                    <CardHeader>
                        <CardTitle>{{ $t('labor_law_rules.edit_rule') }}</CardTitle>
                        <CardDescription>
                            {{ $t('labor_law_rules.edit_rule_description') }}
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        <form @submit.prevent="submit" class="space-y-6">
                            ...
                            <div class="flex justify-end gap-4 pt-4">
                                ...
                                >
                                    <span v-if="form.processing">{{ $t('common.updating') }}</span>
