    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <SettingsLayout>
            <div class="space-y-6">
                <HeadingSmall
                    :title="t('settings.salary_run_approvals')"
                    :description="t('settings.salary_run_approvals_company_picker_description')"
                />

                <Card>
                    <CardHeader>
                        <CardTitle>{{ t('settings.salary_run_approvals_select_company') }}</CardTitle>
                    </CardHeader>
                    <CardContent class="space-y-3">
                        <p v-if="companies.length === 0" class="text-sm text-muted-foreground">
                            {{ t('settings.salary_run_approvals_no_companies') }}
                        </p>
                        ...
                        >
                            ...
                            <Button variant="outline" as-child>
                                <Link :href="route('companies.salary-run-approvals.index', company.id)">
                                    {{ t('settings.salary_run_approvals_manage') }}
                                </Link>
                            </Button>
                        </div>
                    </CardContent>
